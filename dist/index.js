...
const GRAPHQL_URL = GADGET_ENVIRONMENT === "development"
...
// ── MCP Server ───────────────────────────────────────────────────────────────
const server = new Server({ name: `${GADGET_APP}-gadget`, version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "introspect_model",
            description: "List all fields and their types for a Gadget model. Run this first when you're unsure what fields exist on a model.",
            inputSchema: {
                type: "object",
                required: ["model"],
                properties: {
        ...
        {
            name: "query_records",
            description: "Query records from any Gadget model. Specify the model name and a GraphQL field selection. Use introspect_model first to discover available fields.",
            inputSchema: {
                type: "object",
                ...
                properties: {
                    ...
                    fields: {
                        type: "string",
                        description: "GraphQL field selection, e.g. \"id name email createdAt\" or \"id trackingNumber error errorMessage\"",
                    },
                    filter: {
                        type: "object",
                        description: "Gadget filter object, e.g. { \"name\": { \"equals\": \"#59389\" } } or { \"error\": { \"equals\": true } }",
                    },
                    limit: {
                        type: "number",
        ...
        {
            name: "run_graphql",
            description: "Run an arbitrary read-only GraphQL query against the Gadget app. Use this for complex queries with nested relations or custom filtering that query_records can't express.",
            inputSchema: {
                type: "object",
                required: ["query"],
                properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        `, { name: typeName });
            ...
            case "query_records": {
                const { model, fields, filter, limit = 10 } = args;
        ...
        `;
                ...
                const records = connection.edges.map((e) => e.node);
                return {
                    content: [{
                            type: "text",
